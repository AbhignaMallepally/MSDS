...
def recommend_songs(spotify_df, user_name=None, genres=None, artists=None, locale=None, mood=None, num_songs=50, days=30, random_seed=42):
    """
    ...
    return generate_playlist_recommendations(songs_df, matching_songs, num_songs)
...
def generate_playlist_recommendations(songs_df, filtered_DF, num_songs):
    ...
    # Sort & Randomize
    recommended_songs = filtered_DF.sort_values(by='combined_score', ascending=False).head(num_songs).sample(frac=1).reset_index(drop=True)

    return format_song_output(recommended_songs)
#LightFM Code Ends ---------------------------------------------------
